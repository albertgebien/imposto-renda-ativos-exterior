...
class Dolar:
    def __init__(self, data: date = None, periodo: dict = None):
        if self.is_weekend(data):
            raise BancoCentralException('Sábado e Domingo não há cotações: ' + data.strftime("%d/%m/%Y"))
        if self.is_holiday(data):
            raise BancoCentralException('Feriados não há cotações: ' + data.strftime("%d/%m/%Y"))
        self.setUrl(_data=data)
        req = self.getURL()
        resp = req.content.decode("utf-8")
        self.json_string = json.loads(resp)

    def setUrl(self, _data: date = None, _periodo: dict = None):
